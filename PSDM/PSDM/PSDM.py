...
class PSDM:
    def __init__(self, column_data, comp_data, rawdata_df, **kw):
        ...
        if "time" not in column_data.index:
            ...
            if self.time_type == "days":  # base units in minutes
                ...
                self.t_mult = 60.0  ## min_per_hour
            else:
                self.t_mult = 1.0  ## assumes minutes
        else:
        ...
        if "flow_type" not in column_data.index:
            ...
            if self.flow_type == "gal":
                ...
                self.flow_mult = 1.0  ## assumes liters
        else:
        ...
        if "units" not in column_data.index:
            ...
            if self.conc_type == "ug":
                ...
                self.mass_mul = 1.0  ## assumes ug provided
        else:
        ...
        self.duration = kw.get("duration", np.max(rawdata_df.index.values))
        # calculate other fixed values
        self.area = np.pi * (self.diam**2) / 4.0
        self.bedvol = self.area * self.L
        self.ebed = 1.0 - self.rhop / self.rhof  # self.wt/(self.bedvol*self.rhop)
        self.tau = self.bedvol * self.ebed * 60.0 / self.flrt
        self.sf = 0.245423867471 * self.flrt / self.area  # gpm/ft**2
        self.vs = self.flrt / (60.0 * self.area)
        self.re = (2.0 * self.rad * self.vs * self.dw) / (self.ebed * self.vw)
        ...
        if self.brk_type == "force":  # self.brk_df != None and
            ...
            for i in self.brk_df.index:
                self.brk_df.iloc[i]["breakday"] = int(
                    self.brk_df.iloc[i]["breakday"]
                )  # convert everything to interger

        # filter
        self.compounds = kw.get("compounds", list(rawdata_df.columns.levels[1]))
        ...
        self.rk1_store, self.rk2_store, self.rk3_store, self.rk4_store = (
            self.__get_fouling_params()
        )
        ...
        max_time = np.max(
            [self.max_days, self.duration]
        )  # doesn't do anything at the moment for k_fit

        self.time_vals = np.linspace(0, max_time * self.t_mult, 500)
        self.fouling_dict = self.__fouled_k_new(
            self.time_vals
        )  ## removed self.k_data.loc['K'],
        ...
        self.k_by_xn_factor = (
            {}
        )  ## relationship between K and 1/n for a calculated q, should be dictionary of interpolating functions
        self.foul_mult_estimates = {
            comp: 1 for comp in self.compounds
        }  # creates list with 1x factor stored ## used to store estimate of fouling impact

        if len(k_data) == 0:
            self.k_data = pd.DataFrame(
                index=["K", "1/n", "q", "brk", "AveC"], columns=self.compounds
            )
            for comp in self.compounds:
                k, q, classifier, brk, aveC, k_func, foul_mult_est = (
                    self.__calculate_capacity(comp)
                )
            ...
            if self.brk_type == "force":
                tmp_assume = self.brk_df[
                    (self.brk_df["carbon"] == self.carbon)
        ...
        self.mass_transfer = pd.DataFrame(
            0.0, columns=self.compounds, index=["kf", "dp", "ds"]
        )
        ...
        if type(mass_transfer_data) == str:
            ...
            try:
                for comp in mass_transfer_data.columns:
                    for val_type in mass_transfer_data.index:
                        self.mass_transfer.loc[val_type.lower(), comp] = (
                            mass_transfer_data[comp][val_type]
                        )
            except:
            ...
            print(
                "mass_transfer input must be a dictionary: {compound: {kf|dp|ds: value}}"
            )
    ...
    def __fouled_k_new(self, t):
        ...
        if type(t) == np.ndarray:
            ...
            for comp in self.compounds:
                ...
                data_store[comp] = interp1d(t, k_mult_pd, fill_value="extrapolate")
            return data_store
    ...
    def __calculate_capacity(self, compound):
        ...
        xn_f_range = np.arange(
            0.15, 1.001, 0.01
        )  # sets up xn_range for returned interpolating function
        times_to_test = np.arange(self.duration + 1)  ## returns a list of days

        k_function = interp1d(
            xn_f_range, np.ones(len(xn_f_range))
        )  ## creates empty function so something will be returned, need to update
        ...
        f_inf = interp1d(
            infl.index, infl.values, fill_value="extrapolate"
        )  ## need to update
        f_eff = interp1d(
            effl.index, effl.values, fill_value="extrapolate"
        )  ## need to update
        ...
        if self.brk_type == "calc" and not donothing:
            if effl.sum() == 0:  ## no Effluent breakthrough at all
                print(
                    f"Insufficient data to estimate breakthrough for {compound}. Returning minimum K estimate."
                )
                ...
                if (
                    effl.iloc[-1].values[0] > 0
                    and effl.iloc[-1].values[0] / aveC >= 0.25
                ):
            ...
            if (
                effl.max() >= infl.min() and not brk_found
            ):  ## suggests likely crossover/breakthrough
                ...
                for t_idx in times_to_test:  ## searches days to find cross_over
                    if not brk_found:
                        if f_eff(t_idx) >= f_inf(t_idx):
                            ...
                            breakthrough_time = (
                                t_idx - 1
                            ) * 1  # shifts breakthrough to previous day
                            brk_found = True
                ...
                if not brk_found:
                    ...
                    for t_idx in times_to_test:
                        if not brk_found:
                            if (
                                f_eff(t_idx) >= aveC
                            ):  ### must exceed 5% above average C. Replaces correlation finding.
            ...
            if not brk_found:
                ...
                try:
                    params, pcov = curve_fit(
                        ...
                        effl.values,
                        p0=(aveC, 5, 0.1),
                        bounds=((c_bound_low, 0, 0), (c_bound_high, 1e3, 1)),
                        maxfev=10000,
                    )

                    breakthrough_time = np.round(
                        (params[1] + np.log(99)) / params[2], 0
                    )  ## rounds to nearest day, for 99% breakthrough relative to C0/AveC

                    if (
                        breakthrough_time >= self.duration
                    ):  ## rejects solution if breakthrough time doesn't exceed data duration (should have been caught)
                        brk_found = True
                        ...
                        effl.loc[breakthrough_time] = logistic(
                            breakthrough_time, *params
                        )
                ...
                if not brk_found:
                    ...
                    try:
                        ...
                        if effl.values[-1] > 0 and effl.values[-1] / aveC >= 0.25:
                            ...
                            if (
                                nZc < len(yte) and yte[-(nZc + 1)] != 0
                            ):  ## makes sure there is enough data to find non-zero before
            ...
            if brk_found:  ## process breakthrough estimate
                infl_load, _ = quad(
                    f_inf, 0, breakthrough_time
                )  ## influent loading, ignores error
                effl_rem, _ = quad(
                    f_eff, 0, breakthrough_time
                )  ## effluent removal, ignores error

                ## calculate q, ug/g
                q_meas = (
                    (infl_load - effl_rem) * flow_per_day * self.mass_mul / carbon_mass
                )
                ...
                k_function = interp1d(
                    xn_f_range, k_s, fill_value="extrapolate"
                )  ## need to update

            else:
                print(
                    f"WARNING: No Capacity Estimated for {compound}. Assuming data duration for capacity."
                )
            ...
            maxx = self.duration
            brkdy = brk_df[
                (brk_df["carbon"] == self.carbon) & (brk_df["compound"] == compound)
            ]["breakday"].values[0]
            ...
            if brkdy < 2000:
                ...
                if brkdy == 1000:  # report min, but no breakthrough
                    qtmp = (
                        quad(f_infl, 0, maxx, points=xdata)[0]
                    ...
                    qtmp = (
                        quad(f_infl, 0, brkdy, points=xdata)[0]
                ...
                f_infl = interp1d(xdata, infl, fill_value="extrapolate")
                f_effl = interp1d(xdata, effl, fill_value="extrapolate")
                aveC = quad(f_infl, 0, breakthrough_time)[0] / breakthrough_time

                qtmp = (
                    quad(f_infl, 0, intersection)[0] - quad(f_effl, 0, intersection)[0]
                )

                q_meas = flow_per_day * qtmp * self.mass_mul / (carbon_mass)  # ug/g
                k = q_meas / ((aveC * self.mass_mul) ** self.xn)

            breakthrough_code = "supplied"

        ## should return the averaged impact related to K reduction caused by fouling
        foul_mult_est = 1 / np.mean(
            self.fouling_dict[compound](np.arange(breakthrough_time) * self.t_mult)
        )
    ...
    def __reset_column_values(self):
        ...
        self.area = np.pi * (self.diam**2) / 4.0
        self.bedvol = self.area * self.L
        self.ebed = 1.0 - self.wt / (self.bedvol * self.rhop)
        self.tau = self.bedvol * self.ebed * 60.0 / self.flrt
        self.sf = 0.245423867471 * self.flrt / self.area  # gpm/ft**2
        self.vs = self.flrt / (60.0 * self.area)
        self.re = (2.0 * self.rad * self.vs * self.dw) / (self.ebed * self.vw)
        ...
        self.jac_sparse = (
            self.jac_sparse_bup
        )  # spar_Jac(self.num_comps, self.nc, self.nz, self.ne)
    ...
    def run_all(
        self, plot=False, save_file=True, optimize="staged", init_grid=5, init_loop=3
    ):
        """
        Parameters
        ----------
        ...
        orig_test_range = self.test_range * 1.0
        orig_xn_range = self.xn_range * 1.0
        self.optimize_flag = False
        ...
        for compound in self.compounds:
            ...
            xn_range1 = orig_xn_range * 1.0
            k_range1 = orig_test_range * 1.0
            ...
            for loop in range(init_loop + 1):
                ...
                if optimize == "staged":
                    ...
                    xn_range, k_range = generate_grid(
                        grid_num_xn,
                        grid_num_k,
                        loop_num=loop,
                        xn_range=xn_range1,
                ...
                xn_range1 = xn_rng_tmp * 1.0
                k_range1 = k_rng_tmp * 1.0
            ...
            if save_file:
                ...
                with pd.ExcelWriter(
                    self.project_name + "_" + self.carbon + "-" + compound + ".xlsx"
                ) as writer:
                    ...
                    data_tmp = pd.Series(
                        [self.re, best_val_k, best_val_xn, min_val, self.ebct, self.sf],
    ...
    def run_all_smart(
        ...
        opt_flg = self.optimize_flag
        orig_test_range = self.test_range * 1.0
        orig_xn_range = np.round(self.xn_range * 1.0, 5)
        ...
        for compound in self.compounds:  ## search all compounds
            ...
            ssq_storage = pd.DataFrame(
                index=np.round(np.arange(min_k, max_k + scale_k / 2, scale_k), 6),
                columns=np.round(np.arange(0.2, 1 + des_xn / 2, des_xn), 3),
            )

            best_ssq = get_ssq(base_k, best_k_factor, best_xn, compound, ssq_storage)
            ssq_storage.loc[np.round(best_k_factor, 6), np.round(best_xn, 3)] = (
                best_ssq * 1
            )
            ...
            xn_search_idx = np.round(np.linspace(0, xn_idx_max, num=5), 0).astype(
                int
            )  ## should return 0, three middle points, end for rough search.
            ...
            for xn_test in xn_search_idx:
                ...
                if test_ssq < best_ssq:
                    ...
                    xn_idx = (
                        xn_test * 1
                    )  ### resets xn_idx to best starting location of group.
            ...
            while cases_tried <= 5:
                ...
                if np.isnan(ssq_storage.loc[np.round(k_factor, 6), np.round(xn, 3)]):
                    ssq_storage.loc[np.round(k_factor, 6), np.round(xn, 3)] = (
                        base_ssq * 1
                    )
                ...
                if xn_idx < xn_idx_max:
                    ...
                    right_ssq = get_ssq(
                        right_base_k, k_factor, right_xn, compound, ssq_storage
                    )
                    if np.isnan(
                        ssq_storage.loc[np.round(k_factor, 6), np.round(right_xn, 3)]
                    ):
                        ssq_storage.loc[
                            np.round(k_factor, 6), np.round(right_xn, 3)
                        ] = (right_ssq * 1)
                ...
                if xn_idx >= 1:
                    ...
                    left_ssq = get_ssq(
                        left_base_k, k_factor, left_xn, compound, ssq_storage
                    )
                    if np.isnan(
                        ssq_storage.loc[np.round(k_factor, 6), np.round(left_xn, 3)]
                    ):
                        ssq_storage.loc[np.round(k_factor, 6), np.round(left_xn, 3)] = (
                            left_ssq * 1
                        )
                ...
                if k_idx < k_idx_max:
                    ...
                    upper_ssq = get_ssq(
                        center_base_k, upper_k_factor, xn, compound, ssq_storage
                    )
                    if np.isnan(
                        ssq_storage.loc[np.round(upper_k_factor, 6), np.round(xn, 3)]
                    ):
                        ssq_storage.loc[
                            np.round(upper_k_factor, 6), np.round(xn, 3)
                        ] = (upper_ssq * 1)
                ...
                if k_idx >= 1:
                    ...
                    lower_ssq = get_ssq(
                        center_base_k, lower_k_factor, xn, compound, ssq_storage
                    )
                    if np.isnan(
                        ssq_storage.loc[np.round(lower_k_factor, 6), np.round(xn, 3)]
                    ):
                        ssq_storage.loc[
                            np.round(lower_k_factor, 6), np.round(xn, 3)
                        ] = (lower_ssq * 1)
        ...
        self.test_range = orig_test_range * 1.0
        self.xn_range = orig_xn_range * 1.0
    ...
    def run_psdm(self):
        ...
        cb0 = cbo.loc[0]
        cb0[cb0 == 0.0] = 1.0  # if influent at time = 0 is 0, reset to 1
        ...
        difl = (
            (3.595 * 1e-14 * (self.temp + 273.15)) / (vw / 10 * mw ** (0.53))
        ) * 10000.0
        ...
        dp_v = difl / (
            tortu
        )  # *column_prop.loc['epor'] #porosity not used in AdDesignS appendix, removed to match
        ...
        def run():
            ...
            # ==============================================================================
            molar_k = (k_v / mw / ((1.0 / mw) ** xn_v))[self.compounds]

            xni = 1.0 / xn_v

            ds_v = (
                epor * difl * cb0 * psdfr / (tortu * 1e3 * rhop * molar_k * (cb0**xn_v))
            )

            for cdx in self.mass_transfer.columns:
                if self.mass_transfer[cdx]["kf"] > 0.0:
                    kf_v[cdx] = self.mass_transfer[cdx]["kf"]
                if self.mass_transfer[cdx]["dp"] > 0.0:
                    dp_v[cdx] = self.mass_transfer[cdx]["dp"]
                if self.mass_transfer[cdx]["ds"] > 0.0:
            ...
            dgs = (rhop * qe * (1.0 - ebed) * 1000.0) / (ebed * cb0)
            ...
            # depends on kf
            st = (kf_v * (1.0 - ebed) * tau / (ebed * rad))[self.compounds]
            ...
            if water_type != "Organic Free":
                ...
                tortu = (
                    np.ones(len(time_temp)) * self.tortu
                )  ## assumes tortuosity is provided at input and does not change
            ...
            beds_mult = np.multiply(np.tile(br, ThreeDSize), beds_v.reshape(ThreeDSize))
            bedp_v = bedp[compound_list].values
            bedp_mult = np.multiply(np.tile(br, ThreeDSize), bedp_v.reshape(ThreeDSize))
            wr_A = np.tile(wr, TwoDSize)
            az_A = np.tile(az, ThreeDSize)
            stdv_v = stdv[compound_list].values
            dgI_v = dgI[compound_list].values
            stdv_dgI = stdv_v.reshape(TwoDSize) * dgI_v.reshape(TwoDSize)
            dgI_v = dgI_v.reshape(TwoDSize)
            xn_ymA = np.divide(xn_vA, ym_vA)

            for i in range(self.num_comps):
                comp = compound_list[i]
                cinf = interp1d(time, cin[comp].values, fill_value="extrapolate")

                cinfA[i] = cinf(time_temp)
                facA[i] = (1.0 / tortu - d[comp]) / (1.0 - d[comp])
                foulFA[i] = (1.0 / self.fouling_dict[comp](time_temp)) ** xni[comp]
            ...
            def diffun(t, y0):
                ...
                qte2[qte2 == 0.0] = 1e-30  ### set mass to very small number
                yt0_2 = np.multiply(xni_vA, z2)
                yt0_2 = np.tile(
                    yt0_2.sum(axis=0), ThreeDSize
                )  # liquid phase in equilibrium#yt0_c
                ...
                cpore2[qte2 <= 0.0] = 0.0
                cpore2[yt0_2 <= 0.0] = 0.0  # yt0_c
                cpore2[xni_vA * np.asarray(q0_2).astype(np.float64) < -20] = 0.0

                cpore_tmp2 = cpore2[:, nc - 1]
                cpore_tmp2[cpore_tmp2 < 0.0] = 0.0

                cbs2 = np.subtract(
                    y0tmp[:, nc], cpore_tmp2
                )  # reshape vs make outside loop
                ...
                ww2 = np.matmul(wr_A[:, :nd].reshape((self.num_comps, 1, nd)), bb2)
                ...
                tmpnc1 = np.subtract(
                    tmpnc1, ww2[:, :, 1:].reshape(self.num_comps, mc - 1)
                )
                ydot2[:, nc - 1, 1:] = np.divide(
                    tmpnc1, wr_A[:, nc - 1].reshape(TwoDSize)
                )
                ...
                ydotfinal = np.multiply(az_A[:, :, 0], cinfl.reshape(TwoDSize))
            ...
            effluent_locator = [
                (nc + 1) * mc * (i + 1) - 1 for i in range(self.num_comps)
            ]
            ...
            y = solve_ivp(
                ...
                y0,
                method=self.solver,
                jac_sparsity=self.jac_sparse,  # max_step=tstep/3.,\
            )

            # defines interpolating function of predicted effluent
            time_index = (
                y.t / tconv / t_mult
            )  # creates time_array and converts back to dimensions

            cp_tmp = y.y[effluent_locator].transpose()
            cp_tmp[cp_tmp < 0.0] = 0.0  # sets negative values to 0.
            ...
            for i in range(self.num_comps):
                ...
                cp[name] = interp1d(
                    y.t / tconv / t_mult, cp_df[name], fill_value="extrapolate"
                )
            return cp
    ...
    def run_psdm_kfit(self, compound):
        ...
        f_eff = interp1d(
            self.data_df.index,
            self.data_df[self.carbon, compound],
            fill_value="extrapolate",
        )

        ssq_xs = np.arange(
            self.k_data[compound]["brk"]
        )  ## only consider through breakthrough, old: np.max(self.data_df.index))
        ...
        if self.optimize_flag:
            ...
            for i in self.test_range:
                for xn in self.xn_range:
                    ...
                    ssq = (
                        (results[compound](ssq_xs) - f_eff(ssq_xs)) ** 2
                    ).sum()  ## returns sum of squares for assumed daily data with linear interpolated effluent
                    ssqs.loc[i, xn] = ssq
            ...
            if self.k_data_input_type == list:
                if len(self.test_range) == 1 and len(self.xn_range) == 1:
                    ...
                    ssqs = pd.DataFrame(ssq, index=[xn], columns=[k])
                ...
                results = pd.DataFrame(
                    results[compound].y, index=results[compound].x, columns=[compound]
                )
    ...
    def run_psdm_dsfit(self, compound):
        ...
        molar_k = (
            self.k_data[compound]["K"]
            / mw
            / ((1.0 / mw) ** self.k_data[compound]["1/n"])
        )
        ...
        difl = (
            (3.595 * 1e-14 * (self.temp + 273.15)) / (self.vw / 10 * mw ** (0.53))
        ) * 10000.0
        ds_base = (
            self.epor
            * difl
            * cb0
            * self.psdfr
            / (1e3 * self.rhop * cb0 ** self.k_data[compound]["1/n"])
        )
    ...
    def model_uncertainty(
        ...
        self.results = pd.DataFrame(
            index=np.linspace(0, self.duration, num=2000), columns=self.compounds_bup
        )
        ...
        if single:  ## run compounds as single compounds, default
            ...
            for comp in self.compounds_bup:  ## run over all compounds
                ...
                ## get influent/effluent data for single species
                self.data_df = (
                    self.data_bup.transpose().loc[idx[:, comp], :].transpose()
                )

                ## set k_data
                self.k_data = pd.DataFrame(
                    self.k_data_bup[comp].values,
                    index=self.k_data_bup.index,
                    columns=self.compounds,
                )

                temp_results = self.run_psdm()  ## run simulation
                self.results[comp] = temp_results[comp](
                    self.results.index
            ...
            temp_results = (
                self.run_psdm()
            )  ## run simulation, returns dictionary of interpolating functions
        ...
        for comp in self.compounds_bup:
            ...
            self.uncertainty_results.loc[self.results.index, ("upper", comp)] = (
                self.results[comp] * 1
            )
            self.uncertainty_results.loc[self.results.index, ("lower", comp)] = (
                self.results[comp] * 1
            )
        ...
        if capacity != "None":
            test_uncertainty.append(
                {"k": 1 + capacity / 100, "1/n": 1 - capacity / 100}
            )  ## Freundlich K and 1/n work in opposite directions
            test_uncertainty.append(
                {"k": 1 - capacity / 100, "1/n": 1 + capacity / 100}
            )
            compare_bounds = True
        ...
        for test in test_uncertainty:
            ...
            self.mass_transfer = (
                calced_mass_transfer.copy()
            )  ## this always fixes mass transfer to original system flowrates... TODO: is that a problem?
            data = self.data_df
            ...
            for key in test.keys():  ## this should be a dictionary...
                ...
                if key == "mass":  ## adjust mass of media
                    ...
                    if self.wt / volume > self.rhof:
                        self.L = self.wt / (
                            np.pi / 4 * self.diam**2 * self.rhof
                        )  ## may cause shift to EBCT, scaling will always expect base case as EBCT adjustment
                ...
                if key == "C":  ## adjust influent concentration
                    ...
                    self.data_df = (
                        data.copy()
                    )  # overwritten if "single=True", but used by competitive if not.
                    self.k_data.loc["AveC"] *= test[
                        key
                    ]  ## adjust average C for molar K adjustment
                ...
                if key == "qn":  ### adjust capicity (q's) impact on 1/n
                    ...
                    self.k_data.loc["1/n"] = np.log(qs / ks) / np.log(
                        cs * self.mass_mul
                    )
            ...
            self.area = np.pi * (self.diam**2) / 4.0
            self.bedvol = self.area * self.L
            self.ebed = 1.0 - self.rhop / self.rhof
            self.tau = self.bedvol * self.ebed * 60.0 / self.flrt
            self.sf = 0.245423867471 * self.flrt / self.area  # gpm/ft**2
            self.vs = self.flrt / (60.0 * self.area)
            self.re = (2.0 * self.rad * self.vs * self.dw) / (self.ebed * self.vw)
            ...
            if single:  ## run compounds as single compounds, default
                ...
                for comp in self.compounds_bup:  ## run over all compounds
                    ...
                    ## set k_data
                    self.k_data = pd.DataFrame(
                        k_data_loop[comp].values,
                        index=k_data_loop.index,
                        columns=self.compounds,
                    )
                    ...
                    if compare_bounds:
                        ...
                        lower = np.minimum(
                            self.uncertainty_results["lower"][comp].values,
                            temp_results[comp](self.results.index),
                        )
                        upper = np.maximum(
                            self.uncertainty_results["upper"][comp].values,
                            temp_results[comp](self.results.index),
                        )

                        self.uncertainty_results.loc[idx[:, ("lower", comp)]] = (
                            lower * 1
                        )
                        self.uncertainty_results.loc[idx[:, ("upper", comp)]] = (
                            upper * 1
                        )  ## store results in dataframe
                ...
                for comp in temp_results.keys():  ## store results in dataframe
                    ...
                    if compare_bounds:
                        ...
                        self.uncertainty_results["upper"][
                            comp
                        ] = upper  ## store results in dataframe
    ...
    def run_all_MP(self, plot=False, save_file=False):
        ...
        for compound in self.compounds:
            ...
            for k in k_range:
                for xn in xn_range:
                    models.append([deepcopy(self), k * k_mult[xn], xn, compound, k])
            ...
            data_pd = pd.DataFrame(
                retval, columns=["K2", "1/n", "ssq", "compound", "K"]
            )
            ssqs = pd.pivot_table(
                data_pd,
                values=["ssq"],
            ...
            if ~np.isclose(
                ssqs.min().min(), ssqs.max().max()
            ):  # np.floor(ssqs.values[0][0]):
                min_val = find_minimum_df(ssqs)
                best_val_xn = min_val.columns[0]
                best_val_k = min_val.index[0] * k_mult[best_val_xn]
                min_val = min_val.values[0][0]
